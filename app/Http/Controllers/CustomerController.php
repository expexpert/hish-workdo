...
class CustomerController extends Controller
{
    ...
    public function invoiceReviewAction(Request $request)
    {
        ...
        $alreadyValidated = Invoice::where('ref_number', \Auth::user()->invoiceNumberFormatNew($invoice->invoice_number, $invoice->created_at))->where('customer_id', $invoice->customer_id)->exists();
        ...
        if ($request->action == 'VALIDATED' && !$alreadyValidated) {
            ...
            $newInvoice->ref_number     = \Auth::user()->invoiceNumberFormatNew($invoice->invoice_number, $invoice->created_at);
